...
def replace_allergy_codes(dish_text, convert_to_text=True):
    ...
    def convert_match(match):
        ...
        nums = re.findall(r"\d+", raw_inside)
    ...
    # 괄호 안의 숫자 및 마침표 패턴 매칭 (예: (1.5.6) 또는 (2.))
    pattern = r"\(([\d\.]+)\)"
...
try:
    ...
    elif "RESULT" in res_data and res_data["RESULT"]["CODE"] != "INFO-000":
        st.info(
            f"ℹ️ {res_data['RESULT'].get('MESSAGE', '해당 월에 등록된 급식 데이터가 없습니다.')}"
        )
    ...
    for week in month_cal:
        # 월~금 사이에 일자(0이 아닌 값)가 하나라도 있는지 검사
        if not any(week[:5]):
            continue
